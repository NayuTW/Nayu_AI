...
NeuTTS-Air requires reference audio to clone a voice. You can use either raw audio files or pre-encoded reference codes.

#### Option A: Raw Audio Files (Recommended for First-Time Setup)

**Audio Requirements:**
1. **Format**: WAV, MP3, FLAC, or M4A audio file
...
#### Option B: Pre-Encoded Reference Codes (Faster Initialization)

If you have pre-encoded reference codes saved as a `.pt` (PyTorch tensor) file, you can use them directly:

1. **Obtain pre-encoded codes**: These are typically generated by running `tts.encode_reference(audio_path)` once and saving with `torch.save(ref_codes, "reference.pt")`
2. **Set TTS_REF_AUDIO to the .pt file path**: The system will automatically detect and load the pre-encoded codes
3. **Benefits**: Faster initialization since encoding is skipped

**Note**: Pre-encoded codes must match the TTS model being used. If you change models, you'll need to re-encode or use raw audio.
...
# Use the sample audio files
export TTS_REF_AUDIO=/tmp/neutts-air-samples/samples/dave.wav
export TTS_REF_TEXT=/tmp/neutts-air-samples/samples/dave.txt

# OR use pre-encoded codes if available (e.g., dave.pt)
# export TTS_REF_AUDIO=/tmp/neutts-air-samples/samples/dave.pt
# export TTS_REF_TEXT=/tmp/neutts-air-samples/samples/dave.txt
```

Available samples in the repository:
- `samples/dave.wav` and `samples/dave.txt` - audio and transcript
- `samples/dave.pt` - pre-encoded reference codes (if available)
- `samples/jo.wav` - audio only (no text file provided in original repo)
